...
class Form(QtWidgets.QDialog):
    ...
    def startProgressBar(self):  # 프로그래스바 시작
        ...
        xlsx_dir = self.ui.lineEdit_1.text() # 파일 이름 가지고오기
        self.thread.run(xlsx_dir) # 쓰레드 실행, 파일이름을 함수에 전달
    ...
    def run(self, xlsx_dir):
        ...
        try:
            ...
            for row in worksheet.iter_rows():
                ...
                if count > 2:  # 초기 1,2는 제외
                    if percent != a[-2]:  # 이전값 비교했을때 다르면 업데이트
                        print(percent, a[-2])
                        print("입력")
                        self.change_value.emit(percent)

                # if (100*(count/max_row))%1 == 0:
                #     self.change_value.emit(100*(count/max_row))
                #     print(100*(count/max_row))
                # time.sleep(0.1)
